...
[[gnu::always_inline]]
PRIVATE ssize write(FileDescriptor descriptor, const u8* bytes, usize len)
{
    return syscall3(SYSCALL_WRITE, descriptor, (ssize)bytes, (ssize)len);
}

[[gnu::always_inline]]
PRIVATE ssize read(FileDescriptor descriptor, const u8* bytes, usize len)
{
    return syscall3(SYSCALL_READ, descriptor, (ssize)bytes, (ssize)len);
}
...
[[noreturn]] void exit_process(s64 exit_code);

[[noreturn]] void posix_wrapper(long* ptr)
{
    int argc = ptr[0];
...
OpenFlags process_open_file_flags(
